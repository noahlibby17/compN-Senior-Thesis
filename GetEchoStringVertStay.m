function [string, RT] = GetEchoStringVertStay(windowPtr, msg, x, y, textColor, bgColor, useKbCheck, varargin)
% string = GetEchoString(window, msg, x, y, [textColor], [bgColor], [useKbCheck=0], [deviceIndex], [untilTime=inf], [KbCheck args...])
%
%
% Edited version of GetEchoStringVert that keeps already drawn words and textures on the screen
%
%
...
%     fprintf('String = %s\n', string);
    ...
    Screen('Flip', windowPtr, [], 1); % keeps old stuff on screen, UNFORTUNATELY INCLUDING THE LETTERS THAT USER TRIES TO DELETE WHEN TYPING RESPONSE
%     if first
%         WaitSecs(0.01);
%     end
end
