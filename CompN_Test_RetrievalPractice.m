function [mark, track, responses, response] = CompN_Test_RetrievalPractice(window, name, item, texture, msg_x, msg_y, img_loc, textColor, comp_type, mark, track)
...
try
    ...
    for letter = 1:length(name)
        ...
        display = [name(1:letter), '_________''s ', item]; % change this to update the underscores? % change to make more general for other animals
            ...
            if keyIsDown == 1
                ...
                % CHECK to see if the response is close enough to be considered correct. MARK this as 'learned' if this is true
                    ...
                    if check <= 1
                        ...
                        mark = 0; % they guessed incorrectly, repeat this trial
        ...
        if breakflag == 1
            break % breaks out of nested loop and ends trial
        end
        ...
        if comp_type == 6 % high competition animals
            if letter == 3 % if half of the word has been presented, they fail the trial and have to repeat
                % SHOW THE WHOLE WORD %
                Screen('DrawTexture', window, texture, [], img_loc);
                Screen('DrawText', window, [name, '''s ', item], msg_x, msg_y, textColor);
                Screen('Flip', window);
                WaitSecs(5);
